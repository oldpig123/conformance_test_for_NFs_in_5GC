...
class EntityExtractor:
    ...
    def __init__(self, load_llm=True, load_embedder=True):
        ...
        self._setup_models(load_llm=load_llm, load_embedder=load_embedder)
    
    def _setup_models(self, load_llm=True, load_embedder=True):
        """Setup multiple specialized models, distributing them across available GPUs."""
        ...
        llm_device = 0 if gpu_count > 0 else -1
        embedding_device = 1 if gpu_count > 1 else llm_device

        print(f"  Found {gpu_count} GPUs. Assigning LLM to device:{llm_device}, Embedding Model to device:{embedding_device}")

        # Load Entity Extraction LLM
        if load_llm:
            for model_name in LLM_MODEL_OPTIONS:
                try:
                    print(f"  Trying Entity LLM: {model_name} on device:{llm_device}...")
                    if "flan-t5" in model_name:
                        self.entity_llm = pipeline(
                            "text2text-generation",
                            model=model_name,
                            device=llm_device,
                            model_kwargs={"torch_dtype": torch.float16} if llm_device >= 0 else {}
                        )
        ...
        if load_embedder:
            for model_name in EMBEDDING_MODEL_OPTIONS:
                try:
                    print(f"  Trying Embedding Model: {model_name} on device:{embedding_device}...")
                    # For SentenceTransformer, device is passed during initialization
                    device_str = f"cuda:{embedding_device}" if embedding_device >= 0 else "cpu"
                    self.embedding_model = SentenceTransformer(model_name, device=device_str)
